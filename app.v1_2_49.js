...
var ELTA_APP_VERSION_FINAL="1.2.49";
...
function pickDeepValue(obj, keys){
...
function alertKmValue(a,t){
  return pickDeepValue(a,["km","KM","kilometro","kilómetro","kilometraje","kmRuta","km_ruta","progresiva","distancia","kilometros","kilómetros"]) ||
         pickDeepValue(t,["km","KM","kilometro","kilómetro","kilometraje","kmRuta","km_ruta","progresiva","distancia","kilometros","kilómetros"]) || "-";
}
function alertKm(a){ return alertKmValue(a,null); }
function alertKmValueFinalV1244(a,t){ return esc(alertKmValue(a,t)); }
function isAlertPendingV1246(t,a,idx){
  if(typeof isAlertVerified==="function") return !isAlertVerified(t,a,idx);
  return !(a && (a.verificada===true || a.verified===true || String(a.estado||"").toLowerCase()==="verificada"));
}
function openPendingAlertRowsV1246(){
  let rows=[];
  (Array.isArray(trs)?trs:[]).filter(openT).forEach(t=>{
    (t.alerts||[]).forEach((a,idx)=>{
      if(isAlertPendingV1246(t,a,idx)) rows.push({t,a,idx,id:(typeof normalizeAlertIdV1232==="function"?normalizeAlertIdV1232(t,a,idx):`${t.embarque||""}_${idx}`)});
    });
  });
  rows.sort((x,y)=>tv(y.a.time||y.a.fecha||y.a.createdAt||y.a.ts)-tv(x.a.time||x.a.fecha||x.a.createdAt||x.a.ts));
  return rows;
}
function pendingAlertsCount(){ return openPendingAlertRowsV1246().length; }
function trackingAlertsHtml(t){
  ...
  return arr.map(({a})=>{
    ...
    return `<div class="trackingAlertItem"><div class="trackingAlertTop"><span>⚠️ ${tipo}</span><span>${fecha}</span></div><div><b>Localidad:</b> ${esc(typeof alertLoc==="function"?alertLoc(a,t):"-")}</div><div><b>Km:</b> <span class="alertKmHighlight">${esc(alertKmValue(a,t))}</span></div></div>`;
  }).join("");
}
function trackingCardV1246(t){
  let r=ruta(t)||{};
  let last=typeof lastGpsObjectV1237==="function"?lastGpsObjectV1237(t):(typeof lastU==="function"?lastU(t):{});
  let locText=(last?.localidad||last?.ubicacionTexto||last?.locationName||(typeof locFull==="function"?locFull(t):loc(t))||"-");
  return `<div class="trackingCard"><div class="trackingCardTop"><div class="trackingFleetTitle">Flota ${esc(flota(t)||"-")}</div><span class="trackingState">Abierto</span></div><div class="trackingData"><div><b>Emb.:</b> ${esc(t.embarque||"-")}</div><div><b>Chofer:</b> ${esc(typeof driverName==="function"?driverName(t):"-")}</div><div class="locationLine"><b>Ubicación:</b> ${esc(locText)}</div><div><b>Cliente:</b> ${esc(r.cliente||"-")}</div><div><b>Destino:</b> ${esc(r.destino||"-")}</div><div class="trackingReportLine"><b>Últ. GPS:</b> ${fd(last?.time||last?.fecha||last?.createdAt||last?.ts||(typeof transitUpdatedValue==="function"?transitUpdatedValue(t):""))}</div></div><div class="trackingAlerts"><h4>⚠️ Alertas sin verificar</h4>${trackingAlertsHtml(t)}</div></div>`;
}
function renderMapa(){
  if(typeof refreshSeguimientoFilters==="function") refreshSeguimientoFilters();
...
function updateDashboardOpenAlertsV1246(){
  let rows=openPendingAlertRowsV1246();
  let n=rows.length;
  ["kal","headerAlertCount","legAlert"].forEach(id=>{let el=q(id); if(el) el.innerText=n;});
  if(q("dashAlerts")){
    q("dashAlerts").innerHTML=rows.slice(0,3).map(x=>`<div class="alertLine">• ${esc(typeof alertTipo==="function"?alertTipo(x.a):(x.a.tipo||x.a.type||x.a.motivo||"Alerta"))} · Emb. ${esc(x.t.embarque||"-")} · Flota ${esc(flota(x.t)||"-")} · Km ${esc(alertKmValue(x.a,x.t))}</div>`).join("") || '<div class="alertEmpty">Sin alertas sin verificar en tránsitos abiertos.</div>';
  }
  if(typeof updateAlertBellBlink==="function") updateAlertBellBlink();
}
function renderAlerts(){
  let el=q("alertCards")||q("alertList"); if(!el)return;
  let rows=openPendingAlertRowsV1246();
  let by={}; rows.forEach(r=>{let f=flota(r.t)||"-";(by[f]=by[f]||[]).push(r);});
  el.innerHTML=Object.entries(by).map(([fleet,list])=>`<div class="alertFleetCard hasPending"><div class="alertFleetTop"><div class="alertFleetTitle">🚚 Flota ${esc(fleet)}</div><div class="alertFleetSummary"><span class="alertPendingBadge pending">${list.length} pendientes</span></div></div>${list.map(r=>{let rt=ruta(r.t)||{};let id=String(r.id||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return `<div class="alertItemCard pendingAlert"><div class="alertItemContent"><div class="alertItemTop"><span>⚠️ ${esc(typeof alertTipo==="function"?alertTipo(r.a):(r.a.tipo||r.a.type||r.a.motivo||"Alerta"))}</span></div><div class="alertItemMeta"><div><b>Embarque:</b> ${esc(r.t.embarque||"-")}</div><div><b>Km:</b> <span class="alertKmHighlight">${esc(alertKmValue(r.a,r.t))}</span></div><div><b>Fecha/hora:</b> ${typeof alertDateValue==="function"?alertDateValue(r.a):fd(r.a.time||r.a.fecha||r.a.createdAt||r.a.ts)}</div><div><b>Localidad:</b> ${esc(typeof alertLoc==="function"?alertLoc(r.a,r.t):"-")}</div></div><div class="alertTransitDetail"><div><b>Chofer:</b> ${esc(typeof driverName==="function"?driverName(r.t):"-")}</div><div><b>Cliente:</b> ${esc(rt.cliente||"-")}</div><div><b>Origen:</b> ${esc(rt.origen||"-")}</div><div><b>Destino:</b> ${esc(rt.destino||"-")}</div><div><b>Estado:</b> En tránsito</div></div></div><div class="alertItemAction"><button class="alertVerifyBtn" onclick="markAlertVerifiedById('${id}')">Verificar</button></div></div>`;}).join("")}</div>`).join("") || '<div class="alertEmpty">Sin alertas sin verificar en tránsitos abiertos.</div>';
  updateDashboardOpenAlertsV1246();
}
function normalizeMenuV1246(){
  const icons={dash:"🏠",transitos:"🚚",mapa:"📍",clima:"🌦️",unidades:"🚛",alertas:"🔔",clientes:"🏢",abm:"⚙️"};
  document.querySelectorAll(".sideNav button").forEach(btn=>{
    let on=btn.getAttribute("onclick")||""; let id=(on.match(/tab\('([^']+)'\)/)||[])[1]; if(!id)return;
    let label=id==="dash"?"Torre de Control":id==="mapa"?"Seguimiento":id==="abm"?"Configuración":id==="clientes"?"Clientes / Destinos":id.charAt(0).toUpperCase()+id.slice(1);
    btn.innerHTML=`<span class="navIcon">${icons[id]||"•"}</span><b class="navText">${label}</b>`;
    btn.title=label;
  });
}
function updateAdminBoxUser(u,id){
  const name=String(u?.nombre||u?.name||u?.usuario||id||"Administrador").trim();
  const email=String(u?.correo||u?.email||u?.mail||u?.correoElectronico||u?.e_mail||id||"").trim();
  const box=document.querySelector(".adminBox"); if(!box)return;
  const b=box.querySelector("b"), small=box.querySelector("small"), av=box.querySelector(".adminAvatar");
  if(b)b.textContent=name; if(small)small.textContent=email||"-"; if(av)av.textContent="👤";
}
function weatherCardReal(title,subtitle,c,w,extra="",cls="",iconOverride="",statusHtml=""){
  let icon=iconOverride||w.icon||"🌦️";
  let tempValue=(w.temp===undefined||w.temp===null)?"-":w.temp;
  return `<div class="weatherCard ${cls}"><div class="weatherCardTop weatherCardTopV1246"><div class="weatherTitleWrap"><span class="weatherIconBadge">${icon}</span><div class="weatherTitle">${title}</div></div><div class="weatherDesc">${w.icon||""} ${esc(w.desc||"")}</div><div class="weatherTemp"><span>${esc(tempValue)}°</span><span class="weatherTempIcon">${w.icon||""}</span></div></div>${statusHtml?`<div class="weatherStatusLine">${statusHtml}</div>`:""}<div class="weatherData"><div><b>Ubicación:</b> ${esc(subtitle||"-")}</div><div><b>Sensación:</b> ${esc(w.sens)}°</div><div><b>Viento:</b> ${esc(w.wind)} km/h</div><div><b>Actualizado:</b> ${fd(new Date().toISOString())}</div></div>${extra}</div>`;
}
function compactClientCardV1246(x,type){
  let name=type==="cliente"?(x.nombre||x.cliente||x.razonSocial||x.name||x.id||"-"):(x.nombre||x.destino||x.name||x.ubicacion||x.localidad||x.id||"-");
  let contact=x.contacto||x.contact||x.mail||x.email||"-";
  let phone=x.telefono||x.tel||x.phone||"-";
  return `<div class="clientCard compactEntityCard"><div class="clientTop"><div class="clientName">${type==="cliente"?"🏢":"📍"} ${esc(name)}</div></div><div class="clientData compact"><span><b>Contacto:</b> ${esc(contact)}</span><span><b>Teléfono:</b> ${esc(phone)}</span></div></div>`;
}
function renderClients(){ if(q("clientsList")) q("clientsList").innerHTML=(clientes||[]).slice().sort((a,b)=>String(a.nombre||a.id||"").localeCompare(String(b.nombre||b.id||""),"es")).map(c=>compactClientCardV1246(c,"cliente")).join("") || '<div class="clientCard">No hay clientes registrados.</div>'; }
function renderDestinos(){ if(q("destinosList")) q("destinosList").innerHTML=(destinos||[]).slice().sort((a,b)=>String(a.nombre||a.id||"").localeCompare(String(b.nombre||b.id||""),"es")).map(d=>compactClientCardV1246(d,"destino")).join("") || '<div class="clientCard">No hay destinos registrados.</div>'; }
const _login_v1246=login;
login=async function(){ await _login_v1246(); setVersionV1246(); normalizeMenuV1246(); if(currentUserDoc) updateAdminBoxUser(currentUserDoc,currentUserDoc.id); };
const _renderDash_v1246=renderDash;
renderDash=function(){ _renderDash_v1246(); setVersionV1246(); updateDashboardOpenAlertsV1246(); normalizeMenuV1246(); };
const _refresh_v1246=refresh;
refresh=async function(){ await _refresh_v1246(); setVersionV1246(); normalizeMenuV1246(); updateDashboardOpenAlertsV1246(); renderClients(); renderDestinos(); };
document.addEventListener("DOMContentLoaded",()=>{setVersionV1246(); normalizeMenuV1246(); setTimeout(()=>{setVersionV1246(); normalizeMenuV1246(); updateDashboardOpenAlertsV1246();},500);});


/* ===== V1.2.49 - Version sincronizada final y anti-cache ===== */
window.ELTA_APP_VERSION = "1.2.49";
function syncVersionV1249(){
  document.querySelectorAll('span,small,div,p,h1,h2,h3').forEach(function(el){
    if(/Versi[oó]n\s+\d+\.\d+\.\d+/i.test(el.textContent||'')){
      el.textContent=(el.textContent||'').replace(/Versi[oó]n\s+\d+\.\d+\.\d+/i,'Versión 1.2.49');
    }
  });
}
document.addEventListener('DOMContentLoaded',function(){syncVersionV1249(); setTimeout(syncVersionV1249,200); setTimeout(syncVersionV1249,1000);});
if(typeof login==='function'){ const _login_sync_v1249=login; login=async function(){ const r=await _login_sync_v1249.apply(this,arguments); syncVersionV1249(); return r; }; }
if(typeof renderDash==='function'){ const _renderDash_sync_v1249=renderDash; renderDash=function(){ const r=_renderDash_sync_v1249.apply(this,arguments); syncVersionV1249(); return r; }; }
if(typeof refresh==='function'){ const _refresh_sync_v1249=refresh; refresh=async function(){ const r=await _refresh_sync_v1249.apply(this,arguments); syncVersionV1249(); return r; }; }
